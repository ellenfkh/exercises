...
__global__
void
cudaDocontractDataDataScalar_kernel(double * d_left, double * d_right,
		double * d_out,
		int numCells,
		int numLeftFields,
		int numRightFields,
...
cudaDocontractDataDataScalar(double * h_out,
	...
	cudaDocontractDataDataScalar_kernel<<<gridSize, blockSize>>>(d_left,
...
int main(int argc, char* argv[]) {
	...
	/*
	//Cuda arrays
	double * cudaRight = new double[c * p];
	double * cudaLeft = new double[c * p];
	...
	*/
	...
	for (int cl = 0; cl < c; ++c) {
		for (int qp = 0; qp < p; ++qp) {
			cuda_hostLeft(c,p) = inl_c_p(c,p);
			omp_hostLeft(c,p) = inl_c_p(c,p);

			cuda_hostRight(c,p) = inr_c_p(c,p);
			omp_hostRight(c,p) = inr_c_p(c,p);
		}
	}
